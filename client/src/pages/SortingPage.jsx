...
import { insertionSortMeta } from '../visualizers/sorting/algorithms/insertionSort.js'
import { mergeSortMeta } from '../visualizers/sorting/algorithms/mergeSort.js'
import { quickSortMeta } from '../visualizers/sorting/algorithms/quickSort.js'
import { heapSortMeta } from '../visualizers/sorting/algorithms/heapSort.js'
    ...
    <div className="min-h-screen w-full relative overflow-hidden -mt-24 pt-24">
      ...
      <div className="relative z-10 p-6 space-y-6 max-w-[1400px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* ── Left column: Visualizer + Bottom panels ── */}
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-slate-900/40 border border-purple-500/20 backdrop-blur-md rounded-2xl shadow-2xl shadow-purple-900/20 overflow-hidden">
              <div className="flex items-center justify-between px-6 py-3 border-b border-purple-500/20 flex-wrap gap-y-2">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="px-2 py-0.5 rounded-md text-xs font-mono bg-yellow-950/50 border border-yellow-500/30 text-yellow-300">
                    Avg: {selectedAlgo.timeComplexity}
                  </span>
                  ...
                  <span className="flex items-center gap-1.5 text-xs text-slate-400">
                    <span className={`w-2 h-2 rounded-full ${selectedAlgo.stable ? 'bg-emerald-400' : 'bg-rose-400'}`} />
                    {selectedAlgo.stable ? 'Stable' : 'Unstable'}
                  </span>
                </div>
              </div>
              ...
              <div className="p-6 space-y-4">
                ...
                {/* Bar chart */}
                <SortingVisualizer currentStep={currentStep} inputArray={inputArray} />

                {/* Legend + Statistics */}
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500">
                    <LegendDot color="rgb(124,58,237)" label="Default" />
                    <LegendDot color="rgb(59,130,246)" label="Comparing" />
                    <LegendDot color="rgb(244,63,94)" label="Swapping" />
                    <LegendDot color="rgb(34,211,238)" label="Pivot" />
                    <LegendDot color="rgb(16,185,129)" label="Sorted" />
                  </div>
                  <div className="flex items-center gap-5 text-xs">
                    <span className="text-slate-500">Comparisons: <span className="text-cyan-400 font-bold font-mono">{stats.comparisons}</span></span>
                    <span className="text-slate-500">Swaps: <span className="text-pink-400 font-bold font-mono">{stats.swaps}</span></span>
                    <span className="text-slate-500">Step: <span className="text-purple-400 font-bold font-mono">{currentIndex}/{totalSteps > 0 ? totalSteps - 1 : 0}</span></span>
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative w-full h-2 bg-white/10 rounded-full">
                      ...
                      <div
                        ...
                        style={{ left: `calc(${((arraySize - 5) / 45) * 100}% - 8px)` }}
                      />
                    </div>
                  </div>
          ...
          {/* ── Right column: Code Implementation (primary panel) ── */}
...
const badgeColors = {
  green: 'bg-green-950/50 border-green-500/30 text-green-300',
  yellow: 'bg-yellow-950/50 border-yellow-500/30 text-yellow-300',
  red: 'bg-red-950/50 border-red-500/30 text-red-300',
  purple: 'bg-purple-950/50 border-purple-500/30 text-purple-300',
  cyan: 'bg-cyan-950/50 border-cyan-500/30 text-cyan-300',
}

function ComplexityRow({ label, value, color }) {
  return (
    <div className="flex justify-between items-center">
      <span className="text-sm text-slate-400">{label}</span>
      <span
        className={`px-2.5 py-0.5 rounded-md text-xs font-mono border ${badgeColors[color]}`}
      >
        {value}
      </span>
    </div>
