...
import BSTVisualizer from '../visualizers/binary-tree/BSTVisualizer.jsx'
import {
  bstInsert,
  bstDelete,
  bstSearch,
  ...
  resetBSTIdCounter,
  bstMeta,
} from '../visualizers/binary-tree/bstOps.js'
...
export default function TreePage() {
  const [treeRoot, setTreeRoot] = useState(() => buildInitialTree())
  ...
  const handleInsert = () => {
    ...
    if (isNaN(v)) return
    const result = bstInsert(treeRoot, v)
  ...
  const handleDelete = () => {
    const v = parseInt(valueInput.trim(), 10)
    if (isNaN(v)) return
    const result = bstDelete(treeRoot, v)
    setSteps(result.steps)
    setTreeRoot(result.root)
    setValueInput('')
  }
  ...
  const handleSearch = () => {
    ...
    if (isNaN(v)) return
    const result = bstSearch(treeRoot, v)
    setSteps(result.steps)
    setSearchInput('')
  }
  ...
  const codeArray = bstMeta.code[selectedLang] || bstMeta.code.javascript
  ...
  const displayStep = currentStep || {
    state: { root: treeRoot, highlightedNodes: [], insertedNode: null, removedNode: null, foundNode: null },
    highlights: [],
    type: null,
  }
      ...
      <div className="relative z-10 p-6 space-y-6 max-w-[1400px] mx-auto">
        ...
        <div>
          ...
          <p className="text-purple-300">
            Visualize BST operations and traversals step by step
          </p>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-slate-900/40 border border-purple-500/20 backdrop-blur-md rounded-2xl shadow-2xl shadow-purple-900/20 overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                ...
                {/* Tree vis */}
                <BSTVisualizer currentStep={displayStep} />

                {/* Legend */}
                <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500">
                  <LegendDot color="rgb(251,191,36)" label="Comparing" />
            ...
            <div className="bg-slate-900/40 border border-purple-500/20 backdrop-blur-md rounded-2xl shadow-2xl shadow-purple-900/20 overflow-hidden">
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="space-y-2">
                  <label className="text-sm text-purple-300 font-medium">Insert / Delete</label>
                ...
                {/* Search */}
                <div className="space-y-2">
                  <label className="text-sm text-purple-300 font-medium">Search</label>
                  <div className="flex gap-2 flex-wrap">
                    <input
                      type="text"
                      value={searchInput}
                ...
                {/* Traversals */}
                <div className="space-y-2">
                  <label className="text-sm text-purple-300 font-medium">Traversals</label>
                  <div className="flex gap-2 flex-wrap">
