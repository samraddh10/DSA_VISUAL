...
import StackQueueVisualizer from '../visualizers/stack-queue/StackQueueVisualizer.jsx'
import {
  stackPush,
  stackPop,
  stackPeek,
  ...
  stackMeta,
  queueMeta,
} from '../visualizers/stack-queue/stackQueueOps.js'
...
function buildInitialStack() {
  ...
  for (const v of [10, 20, 30]) {
    const result = stackPush(items, v)
    items = result.items
  }
  return items
}
...
function buildInitialQueue() {
  ...
  let items = []
  for (const v of [10, 20, 30]) {
    const result = queueEnqueue(items, v)
    items = result.items
  }
  return items
}
...
export default function StackQueuePage() {
  ...
  const [items, setItems] = useState(() => buildInitialStack())
  ...
  const meta = mode === 'stack' ? stackMeta : queueMeta
  ...
  const handlePush = () => {
    ...
    if (isNaN(v)) return
    if (mode === 'stack') {
      applyResult(stackPush(items, v))
    } else {
      applyResult(queueEnqueue(items, v))
    }
    setValueInput('')
  }
  ...
  const handlePop = () => {
    if (mode === 'stack') {
      applyResult(stackPop(items))
    } else {
      applyResult(queueDequeue(items))
    }
  }

  const handlePeek = () => {
    if (mode === 'stack') {
      const result = stackPeek(items)
      setSteps(result.steps)
    } else {
      const result = queuePeek(items)
      setSteps(result.steps)
    }
  }
  ...
  const handleModeSwitch = (newMode) => {
    setMode(newMode)
    resetSQIdCounter()
    setItems(newMode === 'stack' ? buildInitialStack() : buildInitialQueue())
    setSteps([])
  }

  const handleReset = () => {
    resetSQIdCounter()
    setItems(mode === 'stack' ? buildInitialStack() : buildInitialQueue())
    setSteps([])
  }
  ...
  const codeArray = meta.code[selectedLang] || meta.code.javascript
      ...
      <div className="relative z-10 p-6 space-y-6 max-w-[1400px] mx-auto">
        ...
        <div>
          ...
          <p className="text-purple-300">
            Visualize LIFO and FIFO operations step by step
          </p>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-slate-900/40 border border-purple-500/20 backdrop-blur-md rounded-2xl shadow-2xl shadow-purple-900/20 overflow-hidden">
              <div className="flex items-center justify-between px-6 py-3 border-b border-purple-500/20 flex-wrap gap-y-2">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="px-2 py-0.5 rounded-md text-xs font-mono bg-cyan-950/50 border border-cyan-500/30 text-cyan-300">
                    {mode === 'stack' ? 'Push/Pop' : 'Enqueue/Dequeue'}: O(1)
                  </span>
              ...
              <div className="p-6 space-y-4">
                ...
                {/* Stack/Queue vis */}
                <StackQueueVisualizer currentStep={displayStep} mode={mode} />

                {/* Legend + Size */}
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500">
                    <LegendDot color="rgb(124,58,237)" label="Peeking" />
                    <LegendDot color="rgb(16,185,129)" label="Added" />
                    <LegendDot color="rgb(244,63,94)" label="Removed" />
                  </div>
                  <span className="text-xs text-slate-500">
                    Size:{' '}
                    <span className="text-purple-400 font-bold font-mono">{items.length}</span>
                  </span>
                </div>
